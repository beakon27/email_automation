...
# Check if we're in a serverless environment or PythonAnywhere
app.config['IS_SERVERLESS'] = os.environ.get('VERCEL', False) or os.environ.get('AWS_LAMBDA_FUNCTION_NAME', False)
app.config['IS_PYTHONANYWHERE'] = os.environ.get('PYTHONANYWHERE', False)
app.config['USE_BACKGROUND_THREADS'] = not (app.config['IS_SERVERLESS'] or app.config['IS_PYTHONANYWHERE'])
app.config['USE_S3_STORAGE'] = os.environ.get('USE_S3_STORAGE', 'False').lower() == 'true'

# Set PythonAnywhere writable directories if needed
if app.config['IS_PYTHONANYWHERE']:
    # PythonAnywhere username from environment or default placeholder
    pa_username = os.environ.get('PA_USERNAME', 'YOUR_USERNAME')
    # Update upload folder to use PythonAnywhere writable directory
    app.config['UPLOAD_FOLDER'] = f'/home/{pa_username}/beakon-solutions/uploads'
    # Update log directory
    if not os.path.exists('logs'):
        os.makedirs('logs', exist_ok=True)

# Create upload folder if it doesn't exist and we're not in serverless mode
if not app.config['IS_SERVERLESS'] and not app.config['USE_S3_STORAGE']:
...
# Initialize SocketIO with appropriate configuration for PythonAnywhere
if app.config['IS_PYTHONANYWHERE']:
    # On PythonAnywhere, use 'eventlet' for wider compatibility
    socketio = SocketIO(app, cors_allowed_origins="*", async_mode='eventlet')
else:
    # Default configuration
    socketio = SocketIO(app, cors_allowed_origins="*", async_mode='threading')

# Initialize background scheduler only if not in serverless or PythonAnywhere environment
scheduler = None
if app.config['USE_BACKGROUND_THREADS']:
...
# Configure logging - conditional on environment
if not app.config['IS_SERVERLESS']:
    if not os.path.exists('logs'):
        os.makedirs('logs', exist_ok=True)
